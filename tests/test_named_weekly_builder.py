...
class WeeklyMetadataTests(unittest.TestCase):
    ...
    def test_metadata_uses_portable_epoch_not_git_iso_rendering(self):
        text=METADATA.read_text();self.assertIn("--format=%at%x00%ct",text);self.assertNotIn("--format=%aI%x00%cI",text)
